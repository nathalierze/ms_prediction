...
def sendReport(data, rounded_pred, next_sentence_id, modus):
    ...
    report = predictions(UebungsID=data['UebungsID'], SatzID=next_sentence_id, UserID=data['UserID'], interventiongroup=user.interventiongroup, prediction=rounded_pred, seqMode=data['seqMode'], modus=modus, versionline=data['versionline'], version=satz.Versionsnr, Datum=time)
    report.save()
